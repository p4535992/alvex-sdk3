...
(function() {
	...
	Alvex.DataListsToolbar = function(htmlId) {
		...
		Alvex.DataListsToolbar.superclass.constructor.call(this, htmlId);
	...
	// MOD 4535992
	YAHOO.extend(Alvex.DataListsToolbar, Alfresco.component.DataListToolbar);
	// END MOD 4535992
	/**
	 * Augment prototype with Common Actions module
	 */
	YAHOO.lang.augmentProto(Alvex.DataListsToolbar,
	...
	YAHOO.lang
	.augmentObject(
			Alvex.DataListsToolbar.prototype,
			{
				/**
				...
				options : {
					...
					/**
					 * HtmlId of the field with auto numbering
					 * component. We will check it before new element
					 * submit
					 * 
				...
				//MOD 453592 AVOID double new row form
				avoidDoubleNewRow : 0,
				...
				onReady : function DataListToolbar_onReady() {
					...
					this.widgets.newRowButton = Alfresco.util
					.createYUIButton(this, "newRowButton",
							this.onNewRow, {
						disabled : false,
						value : "create"
					});
					
					this.widgets.exportCsvButton = Alfresco.util
					.createYUIButton(this, "exportCsvButton",
							this.onExportCsv, {
						disabled : false
					});
					/*
					 * if( Dom.get( this.id + '-excelButton' ) ) {
					 * this.widgets.excelButton =
					 * Alfresco.util.createYUIButton(this,
					 * "excelButton", this.onExcelExport, { disabled:
					 * true, value: "create" }); }
					 */
					
					if (Dom.get(this.id + '-rulesButton')) {
						this.widgets.rulesButton = Alfresco.util
						.createYUIButton(this, "rulesButton",
								this.onManageRules, {
							disabled : true,
							value : "create"
						});
					...
					/*
					this.widgets.selectedItems = Alfresco.util
					...
					*/
					...
					this.widgets.printButton = Alfresco.util
					.createYUIButton(this, "printButton", {
						disabled : true
					});
					
					
					this.widgets.rssFeedButton = Alfresco.util
					.createYUIButton(this, "rssFeedButton", {
						disabled : true
					});
					...
					Alvex.DataListsToolbar.superclass.onReady.call(this); // ADD 4535992					
				},
				...
				onExcelExport : function() {
					if (!this.modules.dataGrid){
						this.modules.dataGrid = Alfresco.util.ComponentManager
								.findFirst("Alvex.DataGrid");
					}
					var headers = this.modules.dataGrid.datalistColumns;
					var records = this.modules.dataGrid.widgets.dataTable
					...
					for ( var r in records) {
						...
						for (var i = 0; i < headers.length; i++) {
							...
							if (item)
								if ((headers[i].dataType.toLowerCase() == "datetime")
										|| (headers[i].dataType
												.toLowerCase() == "date")){
									row
											.push(Alfresco.util
													.formatDate(
															Alfresco.util
																	.fromISO8601(data.itemData[key].displayValue),
															"dd.mm.yyyy"));
								}else {
									if (item.length){
					...
					var json = JSON.stringify({
						...
						fileName : "report-" + today.getFullYear()
								+ '-' + (today.getMonth() + 1) + '-'
								+ today.getDate() + '-'
								+ today.getHours() + '-'
								+ today.getMinutes() + '-'
					...
					xmlHttp_req
							.open(
									...
									false);
					if (Alfresco.util.CSRFPolicy
							&& Alfresco.util.CSRFPolicy
									.isFilterEnabled())
						xmlHttp_req.setRequestHeader(
								Alfresco.util.CSRFPolicy.getHeader(),
								Alfresco.util.CSRFPolicy.getToken());
					xmlHttp_req.setRequestHeader("Content-Type",
							"application/json");
					...
					var resp = eval('(' + xmlHttp_req.responseText
							+ ')');
					var downloadUrl = Alfresco.constants.PROXY_URI
							+ "api/node/content/"
							+ Alfresco.util.NodeRef(resp.nodeRef).uri
							+ "/" + resp.name;

					Alfresco.util.PopupManager
							.displayPrompt({
								title : this
										.msg("message.report_ready.title"),
								text : this.msg(
										"message.report_ready.body",
										downloadUrl),
								noEscape : true,
								buttons : [
										{
				 ...
				 *            addListener method
				 */
				onExportCsv : function ExtDataListToolbar_onExportCsv(
						e, p_obj) {
					if (!this.modules.dataGrid){
						this.modules.dataGrid = Alfresco.util.ComponentManager
						.findFirst("Alvex.DataGrid");
					}
					var nodeRef = Alfresco.util
					.NodeRef(this.modules.dataGrid.datalistMeta.nodeRef)
					window.location.href = Alfresco.constants.PROXY_URI
					+ "contentreich/slingshot/datalists/list/node/"
					+ nodeRef.uri + "?format=xlsx";
				...
				onManageRules : function() {
					...
					var rulesURL = Alfresco.constants.URL_PAGECONTEXT
					+ "folder-rules?nodeRef="
					+ this.modules.dataGrid.datalistMeta.nodeRef;
				...
				onNewRow : function DataListToolbar_onNewRow(e, p_obj) {
					...
					if(this.avoidDoubleNewRow==0){
					var datalistMeta = this.modules.dataGrid.datalistMeta, destination = datalistMeta.nodeRef, itemType = datalistMeta.itemType;
					...
					var doBeforeDialogShow = function DataListToolbar_onNewRow_doBeforeDialogShow(
						...
						Alfresco.util.populateHTML([
							...
							this.msg("label.new-row.title") ], [
								p_dialog.id + "-dialogHeader",
								this.msg("label.new-row.header") ]);
					};
					...
					.setOptions(
							{
								...
								doBeforeAjaxRequest : {
									fn : function(config, obj) {
										...
										if (this.options.autoNumbererField == '')
											return true;
										var prop = this.options.autoNumbererField
										.replace(
												/.*_prop_/,
										'')
										.replace(
												/-cntrl/,
										'');
										var number = YAHOO.lang
										.trim(Dom
												.get(this.options.autoNumbererField).value);
										var json = JSON
										.stringify({
										...
										var resp = eval('('
												+ xmlHttp_req.responseText
												+ ')');
										if (resp.success) {
											config.dataObj["prop_"
												+ prop] = resp.id;
										...
										.displayMessage({
											text : this
											.msg("message.duplicate-number.failure")
											+ this
											.msg("message.duplicate-number.nextIdHelp"),
											noEscape : true,
											displayTime : 7
										});

										Dom
										.get(this.options.autoNumbererField).value = resp.id;
										Dom
										.get(this.options.autoNumbererField
												+ "-edit").value = resp.id;
										Dom
										.get(this.options.autoNumbererField
												+ "-display").value = resp.id;

										YAHOO.lang.later(2500,
												createRow,
												createRow.show);

										if (obj.form._toggleSubmitElements)
											obj.form
											._toggleSubmitElements(true);
								...
								onSuccess : {
									fn : function DataListToolbar_onNewRow_success(
											response) {
										YAHOO.Bubbling
										.fire(
												"dataItemCreated",
										...
										.displayMessage({
											text : this
											.msg("message.new-row.success")
										});
									},
									scope : this
								},
								onFailure : {
									fn : function DataListToolbar_onNewRow_failure(
											response) {
										Alfresco.util.PopupManager
										.displayMessage({
											text : this
											.msg("message.new-row.failure")
										});
									},
									scope : this
								}
							}).show();
						this.avoidDoubleNewRow=1;
					}else if(this.avoidDoubleNewRow==1){
						this.avoidDoubleNewRow=0;
					}else{
						//DO NOTHING
					}
				}
...
//				 *            method
//				 */
//				onSelectedItems : function DataListToolbar_onSelectedItems(
//						sType, aArgs, p_obj) {
...
//						var fn = Alfresco.util
//						.findEventClass(eventTarget);
//						if (fn && (typeof this[fn] == "function")) {
//							this[fn].call(this, this.modules.dataGrid
//									.getSelectedItems());
//						}
//					}
//
...
//				 *            type)
//				 */
//				onUserAccess : function DataListToolbar_onUserAccess(
//						layer, args) {
//					var obj = args[1];
...
//				 */
//				onSelectedItemsChanged : function DataListToolbar_onSelectedItemsChanged(
//						layer, args) {
//					if (!this.modules.dataGrid){
//						this.modules.dataGrid = Alfresco.util.ComponentManager
//						.findFirst("Alvex.DataGrid");
//					}
//					if (this.modules.dataGrid) {
//						var items = this.modules.dataGrid
//						.getSelectedItems(), item, userAccess = {}, itemAccess, index, menuItems = this.widgets.selectedItems
...
//									if (menuItem.element.firstChild.rel
//											&& menuItem.element.firstChild.rel !== "") {
//										// Comma-separated indicates and "AND" match
//										actionPermissions = menuItem.element.firstChild.rel
//										.split(",");
//										for (
//												i = 0,
//												ii = actionPermissions.length; i < ii; i++) {
...
//									}
//
//									menuItem.cfg.setProperty(
//											"disabled", disabled);
//								}
//							}
//						}
//						this.widgets.selectedItems.set("disabled",
//								(items.length === 0));
//					}
//				}
			}, true);
