...
(function() {
    ...
    YAHOO.extend(Alvex.OrgchartGroupViewer, Alfresco.component.Base, {
        ...
        updateUI: function() {
            ...
            //MOD 4535992 MANAGE MISSING FIELD ID
            if(!Dom.get(fieldId)){
            	var sel = document.querySelector('[id$=selected-users]');
            	fieldId = document.getElementById(sel.id);
            }
            Dom.get(fieldId).innerHTML = '';
            //END MOD 4535992
            for (m in merged) {
    ...
    {    	
		//MOD 4535992 EXPLICIT CONSTRUTCOR FOR orgChartPicker		
		if(!p_params.orgchartPicker){
	    	if(!this.id){
	    		this.id = p_params.orgchartAdmin.id;
	    	}
	    	if(currentAlvexOrgchartGroupViewer){
	    		this.orgchartGroupViewer = currentAlvexOrgchartGroupViewer;
	    	}
	    	if(!this.orgchartGroupViewer && currentAlvexOrgchartViewer){
	    		this.orgchartGroupViewer = currentAlvexOrgchartViewer;
	    	}
	    	p_params.orgchartPicker = this.orgchartGroupViewer;
			//this.orgchartGroupViewer = new Alvex.OrgchartGroupViewer(this.id).setOptions({
		    //    mandatory: p_params.orgchartAdmin.options.mandatory,
		    //    multipleSelectMode: p_params.orgchartAdmin.options.multipleSelectMode
		    //});//.setMessages(this.messages);
			//p_params.insituEditors = this.orgchartGroupViewer.options.insituEditors;
		}
		//END MOD 4535992 EXPLICIT CONSTRUTCOR FOR orgChartPicker		
    ...
    Alfresco.widget.InsituEditorOrgchartOpen = function(p_editor, p_params)
    {
        ...
        //MOD 4535992 understand why we lose this parameter
        if(p_params.orgchartPicker && p_params.orgchartPicker.msg){
        	this.editIcon.title = Alfresco.util.encodeHTML(p_params.orgchartPicker.msg("alvex.orgchart.unit_members"));
        }else{
        	this.editIcon.title = Alfresco.util.encodeHTML("People");
        }
        //END MOD 4535992
    ...
    YAHOO.extend(Alfresco.widget.InsituEditorOrgchartOpen, Alfresco.widget.InsituEditorIcon,
        {
            onIconClick: function InsituEditorOrgchartOpen_onIconClick(e, obj)
            {
                ...
                if (!(index == null)) {
                	...
                	//MOD 4535992 UNDERSTAND WHY IS UNDEFINED                
                	var group = null;
                	if(obj.params.orgchartPicker){
                		group = obj.params.orgchartPicker.getGroupByIndex(index);
                		obj.params.orgchartPicker.addGroup(group);
                	}else{
                		  obj.params.orgchartPicker = this.orgchartGroupViewer;
	                	  if(!this.orgchart && !obj.params.orgchartAdmin.options.orgchart){
	                		  // Get orgchart branches
	                		  Alfresco.util.Ajax.jsonRequest({
	                			  url: Alfresco.constants.PROXY_URI + "api/alvex/orgchart/branches",
	                			  method: Alfresco.util.Ajax.GET,
	                				  ...
	                				  fn: function (resp)
	                				  {
	                					  ...
	                					  Alfresco.util.Ajax.jsonRequest({
	                						  ...
	                						  successCallback:
	                						  {
	                							  fn: function (resp)
	                							  {
	                								  this.orgchart = resp.json.data[0];
	                			                      if (!this.GroupsArray) {                                                
	                			                          //this.createGroupsArray();
	                			                          var array = [];
	                			                          var crtGrps = function (obj) {
	                			                          ...
	                			                          crtGrps(this.orgchart);
	                			                          this.GroupsArray = array;
	                			                      } else {
	                			                    	  group = this.GroupsArray[index - 1];
	                			                      }
	                			                      group = this.GroupsArray[index - 1];
	                			                      obj.params.orgchartPicker.addGroup(group);
	                							  },
	                	  ...
	                	  }else if(!this.orgchart && obj.params.orgchartAdmin.options.orgchart){
	                		  this.orgchart = obj.params.orgchartAdmin.options.orgchart;
	                          if (!this.GroupsArray) {                                                
	                              //this.createGroupsArray();
	                              var array = [];
	                              var crtGrps = function (obj) {
	                		 ...
	                		 console.error("Can't find a proper orgchart"); 
	                	  }
                	}
                	//END MOD 4535992 UNDERSTAND WHY IS UNDEFINED
