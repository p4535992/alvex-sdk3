<#-- MOD 4535992 -->
<#--
<#include "/org/alfresco/components/component.head.inc">
<@script type="text/javascript" src="${page.url.context}/res/js/components/alvex/managees-workflows/managees-workflows-view.js" />
-->
<#include "/org/alfresco/components/component.head.inc">
<@script type="text/javascript" src="${page.url.context}/res/alvex/alvex-manager-dashboard-tasks/js/components/alvex/managees-workflows/managees-workflows-view.js" />

<@link rel="stylesheet" type="text/css" href="${page.url.context}/res/components/workflow/task-list.css" />
