function main()
{
    ...
    if (json.has("disableAccount"))
    {
        ...
        if (disableAccount && peopleAlvex.isAccountEnabled(userName))
        {
            peopleAlvex.disableAccount(userName);
        }
        else if (!disableAccount && !peopleAlvex.isAccountEnabled(userName))
        {
            peopleAlvex.enableAccount(userName);
        }
    }
    ...
    if (json.has("quota"))
    {
        ...
        peopleAlvex.setQuota(person, quota.toString());
    }
    ...
    if (json.has("removeGroups"))
    {
        ...
        for (var index=0; index<groups.length(); index++)
        {
            ...
            var group = peopleAlvex.getGroup(groupId);
            if (group != null)
            {
                peopleAlvex.removeAuthority(group, person);
            }
        }
    }
    ...
    if (json.has("addGroups"))
    {
        ...
        for (var index=0; index<groups.length(); index++)
        {
            ...
            if (group != null)
            {
                peopleAlvex.addAuthority(group, person);
            }
        }
    }
