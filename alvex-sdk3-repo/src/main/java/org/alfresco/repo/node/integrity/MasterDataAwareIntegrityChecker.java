 ...
 * @author Derek Hulley
 */
public class MasterDataAwareIntegrityChecker extends IntegrityChecker
...
{
    private static Log logger = LogFactory.getLog(MasterDataAwareIntegrityChecker.class);
    
    /** key against which the set of events is stored in the current transaction */
    private static final String KEY_EVENT_SET = "MasterDataAwareIntegrityChecker.EventSet";
    /** key to store the local flag to disable integrity errors, i.e. downgrade to warnings */
    private static final String KEY_WARN_IN_TRANSACTION = "MasterDataAwareIntegrityChecker.WarnInTransaction";
    ...
    /**
     */
    public MasterDataAwareIntegrityChecker()
    {
        this.enabled = true;
    ...
    public void init()
    {
        ...
        PropertyCheck.mandatory("MasterDataAwareIntegrityChecker", "dictionaryService", dictionaryService);
        PropertyCheck.mandatory("MasterDataAwareIntegrityChecker", "nodeService", nodeService);
        PropertyCheck.mandatory("MasterDataAwareIntegrityChecker", "policyComponent", policyComponent);
    ...
    /**
     * @see MasterDataAwarePropertiesIntegrityEvent
    ...
    public void onCreateNode(ChildAssociationRef childAssocRef)
    {
        ...
        if (! storesToIgnore.contains(tenantService.getBaseName(childRef.getStoreRef()).toString()))
        {
            ...
            event = new MasterDataAwarePropertiesIntegrityEvent(
            ...
            Map<QName, Serializable> after)
    {
        if (! storesToIgnore.contains(tenantService.getBaseName(nodeRef.getStoreRef()).toString()))
        {
            ...
            event = new MasterDataAwarePropertiesIntegrityEvent(alvexMasterDataService, nodeService, dictionaryService, nodeRef);
            save(event);
        }
    }
    ...
    public void checkIntegrity() throws IntegrityException
    {
        ...
        boolean warnOnly = MasterDataAwareIntegrityChecker.isWarnInTransaction();
